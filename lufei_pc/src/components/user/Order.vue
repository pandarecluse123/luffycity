  ...
  <div class="user-order">
    ...
    <div class="main">
      ...
      <div class="user-data">
        ...
        <div class="my-order-item" v-for="order in user_order_list">
          <div class="user-data-header">
            <span class="order-time">{{order.created_time|time_format}}</span>
            <span class="order-num">订单号：
                        <span class="my-older-number">{{order.order_number}}</span>
                    </span>
          </div>
          <ul class="nav user-data-list" v-for="course in order.order_course_list">
            <li class="order-info">
            ...
            <li class="order-status">{{order.order_status_info}}</li>
            <li class="order-do">
              <span class="btn btn2" v-if="order.order_status==1"><router-link to="/player?vid=46755f0cb5a1a8b5fbaf316923e925e4_4">去学习</router-link></span>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            gopay(order_num){
                this.$axios.post(`${this.$settings.Host}/payments/${order_num}/alipay/`).then(response=>{
                    location.href=response.data.pay_url
                }).catch(error=>{
                    this.$message('支付出现错误,请联系管理员')
                })
            },
            get_user_order() {
                // 获取当前登录用户的所有订单
                this.$axios.get(`${this.$settings.Host}/user/order/`, {
                    ...
                    this.user_order_list = response.data
